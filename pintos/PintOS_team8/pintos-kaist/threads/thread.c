//=== [1] Include Headers ===//
#include "threads/thread.h"        	// 스레드 구조체 및 스케줄링 함수 정의
#include <debug.h>                 	// ASSERT, PANIC 등 디버깅 매크로
#include <stddef.h>                	// size_t, NULL 등 기본 타입 정의
#include <random.h>                	// 난수 생성 함수
#include <stdio.h>                 	// printf 등 표준 입출력 함수
#include <string.h>                	// memset, memcpy 등 문자열 처리 함수

#include "threads/flags.h"         	// EFLAGS 레지스터 관련 상수
#include "threads/interrupt.h"     	// 인터럽트 비활성화/복구 관련 함수
#include "threads/intr-stubs.h"    	// 인터럽트 핸들러 stub 정의
#include "threads/palloc.h"        	// 페이지 할당 함수
#include "threads/synch.h"         	// 세마포어, 락 등 동기화 프리미티브
#include "threads/vaddr.h"         	// 가상 주소 관련 매크로 및 함수
#include "devices/timer.h"         	// 타이머 장치 관련 함수
#include "intrinsic.h"             	// RSP, 인터럽트 레벨 등 내부 함수

#ifdef USERPROG
#include "userprog/process.h"		// 사용자 프로그램 실행 및 프로세스 관리
#endif


//=== [2] Thread Constants ===//
#define THREAD_MAGIC 0xcd6abf4b		// 스택 오버플로 감지를 위한 고유 매직 넘버
#define THREAD_BASIC 0xd42df210     // 기본 스레드용 식별값 (변경 금지)

#define running_thread() ((struct thread *) (pg_round_down (rrsp ())))	
// 현재 CPU의 RSP(스택 포인터)를 페이지 단위로 내린 후 해당 주소를 thread 포인터로 변환
// 현재 실행 중인 스레드의 커널 스택 기반 주소를 통해 스레드 구조체를 가져옴

#define is_thread(t) ((t) != NULL && (t)->magic == THREAD_MAGIC)
// 주어진 포인터가 유효한 thread 구조체인지 검사 (스택 오버플로 또는 손상 여부 확인용)

#define TIME_SLICE 4
// 스레드에 할당되는 타임 슬라이스(틱 수): 1개의 타임 슬라이스는 4틱


//=== [3] Thread Lists & Global State ===//
static struct list ready_list;         // READY 상태인 스레드들의 큐 (우선순위 기반)
static struct list sleep_list;         // BLOCKED 상태 중 알람으로 잠든 스레드 큐 (wakeup_tick 기준)
static struct list wait_list;		   // 특정 이벤트(예: 프로세스 종료 대기)에 사용되는 스레드 대기 큐
static struct list destruction_req;    // 제거 요청된 스레드 리스트 (할당 해제 대기)

static struct thread *idle_thread;     // CPU가 작업이 없을 때 실행되는 idle 스레드
static struct thread *initial_thread;  // PintOS 부팅 시 최초로 실행되는 메인 스레드

static int64_t awake_closest_tick;     // sleep_list에서 가장 먼저 깨어날 tick 값 (가장 빠른 wakeup_tick)
static unsigned thread_ticks;          // 마지막 스케줄 이후 경과한 tick 수

/* 통계용 틱 카운터 */
static long long idle_ticks;           // idle 스레드가 사용한 총 tick 수
static long long kernel_ticks;         // 커널 모드 스레드가 사용한 총 tick 수
static long long user_ticks;           // 유저 모드 스레드가 사용한 총 tick 수

/* tid 할당용 락 */
static struct lock tid_lock;		   // TID 할당 시 동기화를 위한 락

/* 스케줄러 설정 */
bool thread_mlfqs;					   // MLFQ 스케줄러 사용 여부 (기본: false)


//=== [4] GDT 초기화용 커널 전용 GDT ===//
static uint64_t gdt[3] = {			   // 커널 전용 GDT(Global Descriptor Table) 엔트리 정의
    0,                                 // NULL segment
...
/* ------------------ Scheduler ------------------ */
static void do_schedule (int status);					// 현재 스레드 종료 처리 및 다음 스레드 스케줄링
static void schedule (void);							// next_thread_to_run() 호출 및 컨텍스트 스위칭
static struct thread *next_thread_to_run (void);		// 다음 실행할 스레드를 ready_list에서 선택

/* ------------------ Thread Lifecycle ------------------ */
static void kernel_thread (thread_func *, void *aux);	// 커널 스레드 시작 함수
static void idle (void *idle_started_);					// idle 스레드 함수
static void init_thread (struct thread *t, const char *name, int priority);	// 스레드 구조체 초기화
static tid_t allocate_tid (void);						// 고유 스레드 ID(TID) 할당

/* ------------------ Ready/Sleep Queue Compare Functions ------------------ */
bool cmp_priority (const struct list_elem *a, const struct list_elem *b, void *aux);
// ready_list 우선순위 비교 함수 (높은 priority 우선)

static bool cmp_wakeup_tick (const struct list_elem *a, const struct list_elem *b, void *aux UNUSED);
// sleep_list의 wakeup_tick 비교 함수 (가장 빠른 tick 우선)

/* ------------------ Debug Utilities ------------------ */
// static void debug_print_thread_lists (void);    		// 디버깅용: 현재 리스트 상태 출력
...
thread_create (const char *name, int priority, thread_func *function, void *aux) 
{
	...
	t->tf.R.rdi = (uint64_t) function;      // 첫 번째 인자로 실행할 함수 전달 → initd
	t->tf.R.rsi = (uint64_t) aux;           // 두 번째 인자로 함수 인자 전달 → fn_copy → 즉, "echo hello"
