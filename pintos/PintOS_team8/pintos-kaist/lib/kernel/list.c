 ...
 * ⏱ 주요 함수 시간 복잡도 요약
 *
 * - list_push_front / list_push_back        : O(1)
 * - list_insert_ordered                     : O(n)
 * - list_remove                             : O(1)
 * - list_size                               : O(n)
 * - list_sort (Merge Sort)                  : O(n log n)
 * - list_unique                             : O(n)
 ******************************************************************************/
...
/*************************************************************
 * 📦 list_elem 구조와 list_entry 매크로
 *
 * - struct list_elem은 사용자 구조체 내부에 포함되는 노드 구조
 *   예: struct thread { ..., struct list_elem elem; };
 *
 * - list_entry(P, TYPE, MEMBER):
 *   리스트 요소 P가 TYPE 구조체의 MEMBER 필드일 때,
 *   해당 TYPE* 주소를 얻는 매크로 (포인터 연산 기반)
 *
 * 예:
 *   struct list_elem *e = list_pop_front(&ready_list);
 *   struct thread *t = list_entry(e, struct thread, elem);
 * 
 * ⚠️ list_remove() 이후 해당 요소에 list_next/prev 사용 금지 (비정의 동작 발생)
 *************************************************************/
struct list_elem *
 ...
 * e.g. thread_unblock()에서 ready_list에 스레드를 우선순위 순으로 삽입할 때
